...
const I18N = {
  ...
  'zh-CN': {
    ...
    management: '管理', rmRanking: 'RM 排名', baseRanking: '网点 / 加盟网点排名', baseGlobalCut: '按网点与加盟网点筛选', baseCount: '{n} 个网点', distribution: '分布', ownVsFranchise: '直营网点 vs 加盟网点', share: '占比', bases: '网点数', topBase: '主要网点', merchandiseValue: '货值', pnrRate: 'PNR 率', variationD1: '较前一日变化', mostOffendingBase: '投诉最多网点',
...
function refreshSelectLabels() {
  const ids = ['regionalFilter','supervisorFilter','rmFilter','stationFilter','baseFilter','atendimentoFilter','baseRegionalFilter','baseSupervisorFilter','baseStationFilter','baseBaseFilter','baseAtendimentoFilter','editorRegionalFilter','editorSupervisorFilter','editorRmFilter','editorStationFilter','editorBaseFilter','editorAtendimentoFilter','chartRegionalFilter'];
  ids.forEach(id => {
...
async function loadFilters({ preserve = false } = {}) {
  const old = {};
  if (preserve) ['startDate','endDate','regionalFilter','supervisorFilter','rmFilter','stationFilter','baseFilter','atendimentoFilter','baseStartDate','baseEndDate','baseRegionalFilter','baseSupervisorFilter','baseStationFilter','baseBaseFilter','baseAtendimentoFilter','chartStartDate','chartEndDate','chartRegionalFilter'].forEach(id => { if ($(id)) old[id] = $(id).value; });
  ...
  const mainMap = [
    ...
    ['baseRegionalFilter','regional'],['baseSupervisorFilter','supervisor'],['baseStationFilter','station'],['baseBaseFilter','base'],['baseAtendimentoFilter','atendimento'],
  ...
  ['startDate','endDate','baseStartDate','baseEndDate','editorStartDate','editorEndDate','chartStartDate','chartEndDate'].forEach(id => {
  ...
  if (!preserve || !$('baseStartDate').value) { $('baseStartDate').value = payload.date_max || ''; $('baseEndDate').value = payload.date_max || ''; }
...
function baseQueryString() {
  const params = new URLSearchParams();
  const pairs = [['start_date','baseStartDate'],['end_date','baseEndDate'],['regional','baseRegionalFilter'],['supervisor','baseSupervisorFilter'],['station','baseStationFilter'],['base','baseBaseFilter'],['atendimento','baseAtendimentoFilter']];
  pairs.forEach(([key,id]) => { const v=$(id).value; if (v && v !== '__all__') params.set(key,v); });
  if (state.baseClickFilters.driver) params.set('driver', state.baseClickFilters.driver);
  if (state.baseClickFilters.order_source) params.set('order_source', state.baseClickFilters.order_source);
  if (state.baseClickFilters.has_base) params.set('has_base','1');
  if (state.baseClickFilters.has_driver) params.set('has_driver','1');
  return params.toString();
}

async function loadBaseDashboard() {
  if (state.baseDashboardLoading) return;
  state.baseDashboardLoading = true;
  try {
...
function renderBaseDashboard(data) {
  const k = data.kpis || {};
  $('baseKpiTotal').textContent = formatNum(k.total); $('baseKpiOwn').textContent = formatNum(k.own); $('baseKpiFranchise').textContent = formatNum(k.franchise); $('baseKpiBases').textContent = formatNum(k.bases); $('baseKpiDrivers').textContent = formatNum(k.drivers);
  $('baseKpiOwnPct').textContent = pct(k.total ? k.own/k.total*100 : 0); $('baseKpiFranchisePct').textContent = pct(k.total ? k.franchise/k.total*100 : 0);
  $('baseCountBadge').textContent = t('baseCount', {n: formatNum(k.bases || 0)});
  $('baseRateDateBadge').textContent = data.rate_reference_date ? t('rateOn', {date: formatDate(data.rate_reference_date)}) : '—';
  renderBaseRankingTable(data.base_ranking || []);
  renderBaseStationTable(data.station_summary || []);
  renderBaseRankingList('baseTopBases', data.top_bases || [], 'base');
  renderBaseRankingList('baseTopDrivers', data.top_drivers || [], 'driver');
  renderBaseRankingList('baseTopOrigins', data.top_origins || [], 'order_source');
  renderBaseDaily(data.daily || []);
  renderBaseClickFilterBar();
}
...
function renderBaseStationTable(rows){
  const body=$('baseStationTableBody');body.innerHTML='';
  rows.forEach(row=>{const tr=document.createElement('tr');tr.className='click-row';tr.tabIndex=0;tr.innerHTML=`<td><span class="station-type-cell"><i class="station-dot ${row.station==='Franquia'?'muted':''}"></i>${escapeHtml(translateKnownValue(row.station))}</span></td><td class="num">${formatNum(row.count)}</td><td class="num">${pct(row.share)}</td><td class="num">${formatNum(row.bases)}</td><td>${escapeHtml(row.top_base)}</td>`;const apply=async()=>{$('baseStationFilter').value=row.station;await loadBaseDashboard();};tr.addEventListener('click',apply);tr.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();apply();}});body.appendChild(tr);});
}

function renderBaseRankingList(id,rows,filterKey){
  const root=$(id);root.innerHTML='';if(!rows.length){root.innerHTML=`<div class="empty-state">${escapeHtml(t('noDataPeriod'))}</div>`;return;}
  const max=Math.max(...rows.map(r=>r.count),1);
  rows.forEach((row,i)=>{const item=document.createElement('button');item.type='button';item.className='rank-item';item.innerHTML=`<span class="rank-pos">${String(i+1).padStart(2,'0')}</span><span class="rank-main"><strong class="rank-label">${escapeHtml(row.label)}</strong><span class="rank-track"><i style="width:${Math.max(4,row.count/max*100)}%"></i></span></span><strong class="rank-count">${formatNum(row.count)}</strong>`;item.addEventListener('click',async()=>{if(filterKey==='base')$('baseBaseFilter').value=row.label;else state.baseClickFilters[filterKey]=state.baseClickFilters[filterKey]===row.label?'':row.label;await loadBaseDashboard();});root.appendChild(item);});
}

function renderBaseDaily(rows){
  const root=$('baseDailyChart');root.innerHTML='';if(!rows.length){root.innerHTML=`<div class="empty-state">${escapeHtml(t('noDaily'))}</div>`;$('basePeriodBadge').textContent='—';return;}
  const max=Math.max(...rows.map(r=>r.count),1);$('basePeriodBadge').textContent=`${formatDate(rows[0].date)} — ${formatDate(rows[rows.length-1].date)}`;
  const bars=document.createElement('div');bars.className='daily-bars';rows.forEach(row=>{const btn=document.createElement('button');btn.type='button';btn.className='daily-bar';btn.title=`${formatDate(row.date)} · ${formatNum(row.count)} PNR`;btn.innerHTML=`<span class="daily-value">${formatNum(row.count)}</span><i style="height:${Math.max(8,row.count/max*100)}%"></i><small>${formatDate(row.date).slice(0,5)}</small>`;btn.addEventListener('click',async()=>{$('baseStartDate').value=row.date;$('baseEndDate').value=row.date;baseMarkChip('custom');await loadBaseDashboard();});bars.appendChild(btn);});root.appendChild(bars);
}

function renderBaseClickFilterBar(){
  const chips=[];if(state.baseClickFilters.driver)chips.push(['driver',`${t('driver')}: ${state.baseClickFilters.driver}`]);if(state.baseClickFilters.order_source)chips.push(['order_source',`${t('orderOrigin')}: ${state.baseClickFilters.order_source}`]);if(state.baseClickFilters.has_base)chips.push(['has_base',t('withBase')]);if(state.baseClickFilters.has_driver)chips.push(['has_driver',t('withDriver')]);
  $('baseClickFilterBar').hidden=!chips.length;const root=$('baseClickFilterChips');root.innerHTML='';chips.forEach(([key,label])=>{const b=document.createElement('button');b.type='button';b.className='click-filter-chip';b.textContent=`${label} ×`;b.addEventListener('click',async()=>{if(key==='has_base'||key==='has_driver')state.baseClickFilters[key]=false;else state.baseClickFilters[key]='';await loadBaseDashboard();});root.appendChild(b);});
}
async function clearBaseClickFilters(){state.baseClickFilters={driver:'',order_source:'',has_base:false,has_driver:false};await loadBaseDashboard();}
async function applyBaseKpiCrossFilter(type){if(type==='total'){state.baseClickFilters={driver:'',order_source:'',has_base:false,has_driver:false};$('baseStationFilter').value='__all__';}else if(type==='own'){$('baseStationFilter').value='Própria';}else if(type==='franchise'){$('baseStationFilter').value='Franquia';}else if(type==='bases'){state.baseClickFilters.has_base=!state.baseClickFilters.has_base;}else if(type==='drivers'){state.baseClickFilters.has_driver=!state.baseClickFilters.has_driver;}await loadBaseDashboard();}
function baseMarkChip(range){document.querySelectorAll('.base-quick-dates .chip').forEach(b=>b.classList.toggle('active',b.dataset.baseRange===range));}
function baseRangeDates(range){const max=state.filters?.date_max;if(!max)return;if(range==='day'){$('baseStartDate').value=max;$('baseEndDate').value=max;}else if(range==='7'){$('baseStartDate').value=addDays(max,-6);$('baseEndDate').value=max;}else if(range==='30'){$('baseStartDate').value=addDays(max,-29);$('baseEndDate').value=max;}else{$('baseStartDate').value=state.filters.date_min||'';$('baseEndDate').value=max;}}
async function resetBaseFilters(){['baseRegionalFilter','baseSupervisorFilter','baseStationFilter','baseBaseFilter','baseAtendimentoFilter'].forEach(id=>$(id).value='__all__');state.baseClickFilters={driver:'',order_source:'',has_base:false,has_driver:false};baseRangeDates('day');baseMarkChip('day');await loadBaseDashboard();}
let baseReloadTimer=null;function scheduleBaseFilterReload(){clearTimeout(baseReloadTimer);baseMarkChip('custom');baseReloadTimer=setTimeout(()=>loadBaseDashboard().catch(err=>toast(err.message,'error')),160);}
function exportBaseTables(){const qs=baseQueryString();const url=`/api/export?${qs}${qs?'&':''}mode=bases&lang=${encodeURIComponent(state.lang)}`;window.location.href=url;toast(t('exportStarted'),'success');}
...
function markChip(range){document.querySelectorAll('.quick-dates .chip[data-range]').forEach(b=>b.classList.toggle('active',b.dataset.range===range));}
...
function renderRmLineChart(data){
  ...
  const palette=['#ed1c24','#2563eb','#16a34a','#9333ea','#ea580c','#0891b2','#d97706','#4f46e5','#db2777','#059669','#0f766e','#7c3aed'];
  const valid=series.flatMap(s=>s.points||[]).filter(p=>p.rate!==null&&p.rate!==undefined);
  if(!dates.length||!series.length||!valid.length){root.innerHTML=`<div class="empty-state chart-empty">${escapeHtml(t('noChartData'))}</div>`;return;}

  // Cada RM ocupa sua própria faixa horizontal. Isso elimina a sobreposição entre linhas,
  // pontos e rótulos quando vários RMs são exibidos ao mesmo tempo.
  const globalMax=Math.max(...valid.map(p=>Number(p.rate)||0),1);
  const pointGap=128;
  const padX=52;
  const plotWidth=Math.max(860,(dates.length-1)*pointGap+padX*2);
  const rowHeight=132;
  const yTop=46;
  const yBottom=104;
  const y=v=>yBottom-(Number(v||0)/globalMax)*(yBottom-yTop);
  const x=i=>padX+(dates.length===1?(plotWidth-padX*2)/2:i*pointGap);
  const matrix=document.createElement('div');matrix.className='spark-matrix';

  series.forEach((s,si)=>{
    const color=palette[si%palette.length];
    const row=document.createElement('div');row.className='spark-series-row';row.style.minWidth=`${plotWidth+240}px`;
    const label=document.createElement('div');label.className='spark-series-label';
    const available=(s.points||[]).filter(p=>p.rate!==null&&p.rate!==undefined);
    const latest=available.length?available[available.length-1]:null;
    label.innerHTML=`<span class="spark-series-name"><i style="background:${color}"></i><strong>${escapeHtml(s.rm)}</strong></span><small>${latest?`${formatRate(latest.rate)} · ${formatNum(latest.complaints)} ${escapeHtml(t('complaintsShort'))}`:'—'}</small>`;

    const canvas=document.createElement('div');canvas.className='spark-series-canvas';canvas.style.width=`${plotWidth}px`;
    const pts=(s.points||[]).map((p,i)=>({...p,i})).filter(p=>p.rate!==null&&p.rate!==undefined);
    let svg=`<svg viewBox="0 0 ${plotWidth} ${rowHeight}" width="${plotWidth}" height="${rowHeight}" role="img" aria-label="${escapeHtml(s.rm)}">`;
    // Guias leves ajudam a leitura sem misturar as séries.
    [0,.5,1].forEach(fr=>{const yy=yTop+(yBottom-yTop)*fr;svg+=`<line x1="0" y1="${yy}" x2="${plotWidth}" y2="${yy}" class="spark-row-grid"/>`;});
    dates.forEach((_,i)=>{const xx=x(i);svg+=`<line x1="${xx}" y1="${yTop-8}" x2="${xx}" y2="${yBottom+7}" class="spark-day-guide"/>`;});
    if(pts.length){const d=pts.map((p,j)=>`${j?'L':'M'} ${x(p.i).toFixed(1)} ${y(p.rate).toFixed(1)}`).join(' ');svg+=`<path d="${d}" fill="none" stroke="${color}" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>`;}
    pts.forEach(p=>{
      const xx=x(p.i),yy=y(p.rate);const labelY=Math.max(17,yy-25);const labelW=84;
      svg+=`<g class="chart-point"><rect x="${xx-labelW/2}" y="${labelY-13}" width="${labelW}" height="30" rx="7" class="spark-point-label-bg"/><circle cx="${xx}" cy="${yy}" r="6.5" fill="#fff" stroke="${color}" stroke-width="3.5"/><text x="${xx}" y="${labelY-1}" text-anchor="middle" class="spark-point-label" fill="${color}"><tspan x="${xx}" font-weight="850">${formatRate(p.rate)}</tspan><tspan x="${xx}" dy="12">${formatNum(p.complaints)} ${escapeHtml(t('complaintsShort'))}</tspan></text><title>${escapeHtml(s.rm)} · ${escapeHtml(formatDate(p.date))} · ${formatRate(p.rate)} · ${formatNum(p.complaints)} ${escapeHtml(t('complaintsShort'))} · ${formatNum(p.deliveries)} ${escapeHtml(t('deliveriesShort'))}</title></g>`;
    });
    svg+='</svg>';canvas.innerHTML=svg;row.appendChild(label);row.appendChild(canvas);matrix.appendChild(row);
  });

  const axis=document.createElement('div');axis.className='spark-date-row';axis.style.minWidth=`${plotWidth+240}px`;
  const spacer=document.createElement('div');spacer.className='spark-series-label spark-axis-label';spacer.textContent=t('date');
  const axisCanvas=document.createElement('div');axisCanvas.className='spark-series-canvas spark-date-canvas';axisCanvas.style.width=`${plotWidth}px`;
  let axisSvg=`<svg viewBox="0 0 ${plotWidth} 58" width="${plotWidth}" height="58" aria-hidden="true">`;
  dates.forEach((d,i)=>{const xx=x(i);axisSvg+=`<line x1="${xx}" y1="0" x2="${xx}" y2="8" class="spark-axis-tick"/><text x="${xx}" y="28" text-anchor="middle" class="spark-date-label">${escapeHtml(formatDate(d))}</text>`;});
  axisSvg+='</svg>';axisCanvas.innerHTML=axisSvg;axis.appendChild(spacer);axis.appendChild(axisCanvas);matrix.appendChild(axis);
...
function bindEvents(){
  ...
  document.querySelectorAll('.quick-dates .chip[data-range]').forEach(btn=>btn.addEventListener('click',async()=>{rangeDates(btn.dataset.range);markChip(btn.dataset.range);await loadDashboard();}));
  ...
  ['baseStartDate','baseEndDate','baseRegionalFilter','baseSupervisorFilter','baseStationFilter','baseBaseFilter','baseAtendimentoFilter'].forEach(id=>$(id).addEventListener('change',scheduleBaseFilterReload));
